...
checkout_line=$(rg -n '^ok "Template commit verified:' "$setup" | cut -d: -f1)
link_line=$(rg -n 'open "\$CLAUDE_DEEP_LINK"' "$setup" | cut -d: -f1)
...
if rg -n -- '--dangerously-skip-permissions|--permission-mode[ =](bypassPermissions|dontAsk)' "$setup"; then
