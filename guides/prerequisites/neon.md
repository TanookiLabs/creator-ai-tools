...
The `DATABASE_URL` uses the connection pooler (the hostname contains `-pooler`). The `DIRECT_URL` is for explicit, reviewed Prisma migrations. Keep both values server-only and pointed at the same database branch.
